...
class Parser {
    ...
    parse(text) {
        ...
        var fragment = null;
        for (var line of text.split('').reverse().join('').split('\n')) {
     ...
     */
    _isSignature(text) {
        return this._signatureRegex.test(text.split('').reverse().join(''));
    }
     ...
     */
    _isQuoteHeader(text) {
        return filter(this._quoteHeadersRegex, exp => exp.test(text.split('').reverse().join(''))).length > 0;
    }
}
