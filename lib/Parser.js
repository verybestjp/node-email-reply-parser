...
var reverse = require("lodash-es/reverse");
...
class FragmentDto {
     ...
     */
    toFragment() {
        var content = Array.from(this.lines.join('\n')).reverse().join('').replace(/^\n/, '');
...
class Parser {
    ...
    parse(text) {
        ...
        var fragment = null;
        for (var line of Array.from(text).reverse().join('').split('\n')) {
     ...
     */
    _isSignature(text) {
        return this._signatureRegex.test(Array.from(text).reverse().join(''));
    }
     ...
     */
    _isQuoteHeader(text) {
        return filter(this._quoteHeadersRegex, exp => exp.test(Array.from(text).reverse().join(''))).length > 0;
    }
}
